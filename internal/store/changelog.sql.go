...
SELECT id, workspace_id, title, subtitle, source_id, source_type, logo_src, logo_link, logo_alt, logo_height, logo_width, created_at FROM changelogs
WHERE workspace_id = $1 AND id =$2
`
...
func (q *Queries) GetChangelog(ctx context.Context, arg GetChangelogParams) (Changelog, error) {
...
const getChangelogSource = `-- name: GetChangelogSource :one
...
type GetChangelogSourceParams struct {
	WorkspaceID string
	ID          int64
}

type GetChangelogSourceRow struct {
	Changelog       Changelog
	ChangelogSource ChangelogSource
}

func (q *Queries) GetChangelogSource(ctx context.Context, arg GetChangelogSourceParams) (GetChangelogSourceRow, error) {
	row := q.db.QueryRow(ctx, getChangelogSource, arg.WorkspaceID, arg.ID)
	var i GetChangelogSourceRow
	err := row.Scan(
		&i.Changelog.ID,
...
func (q *Queries) ListChangelogs(ctx context.Context, workspaceID string) ([]Changelog, error) {
